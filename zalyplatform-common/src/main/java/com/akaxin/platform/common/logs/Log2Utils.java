 ...
 */
package com.akaxin.platform.common.logs;
 ...
 */
public class Log2Utils extends Log2Creater {
	...
	public static void requestDebugLog(Logger logger, Command command, String requestStr) {
		logger.debug("client={} globalUserId={} action={} command={} request={}", command.getClientIp(),
				command.getGlobalUserId(), command.getAction(), command.toString(), requestStr);
	}

	public static void requestErrorLog(Logger logger, Command command, Throwable t) {
		logger.error(StringHelper.format("client={} globalUserId={} action={} uri={} error", command.getClientIp(),
				command.getGlobalUserId(), command.getAction(), command.getUri()), t);
	}

	public static void requestErrorLog(Logger logger, Command command, Class<?> clazz, Throwable t) {
		logger.error(
				StringHelper.format("client={} globalUserId={} action={} uri={} {} error", command.getClientIp(),
						command.getGlobalUserId(), command.getAction(), command.getUri(), clazz.getClass().getName()),
				t);
	}
	...
	public static void dbDebugLog(Logger logger, long startTime, Object result, String sql, Object... objects) {
		...
		FormattingTuple format = MessageFormatter.arrayFormat(messagePattern, objects);
		logger.debug("[DB] cost:{}ms result:{} sql:{}", System.currentTimeMillis() - startTime, result,
				format.getMessage());
	}
